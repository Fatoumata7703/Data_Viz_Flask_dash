"""Callbacks pour la section Organisation"""
...
def register_organization_callbacks(app, df):
    ...
    @app.callback(
        [Output('organization-stat-departements', 'children'),
         Output('organization-stat-cout-total', 'children'),
         Output('organization-stat-duree-moyenne', 'children'),
         Output('organization-stat-cout-jour', 'children')],
    ...
    def update_organization_stats(departement, maladie, traitement, sexe, age_min, age_max):
        ...
        total_departements = len(filtered_df['Departement'].unique()) if len(filtered_df) > 0 else 0
    ...
    @app.callback(
        [Output('graph-cout-departement', 'figure'),
         Output('graph-duree-departement', 'figure'),
         Output('graph-cout-jour-departement', 'figure')],
        [Input('filter-departement', 'value'),
         Input('filter-maladie', 'value'),
         Input('filter-traitement', 'value'),
         Input('filter-sexe', 'value'),
    ...
    def update_organization_graphs(departement, maladie, traitement, sexe, age_min, age_max):
        """Met à jour les graphiques de l'organisation"""
        ...
        # Vérifier si dept_stats est vide
        if len(dept_stats) == 0:
            empty_fig = go.Figure()
            empty_fig.update_layout(
                title='Aucune donnée disponible',
        ...
        dept_stats_cout = dept_stats.sort_values('CoutTotal', ascending=False)
        dept_stats_duree = dept_stats.sort_values('DureeMoyenne', ascending=False)
        dept_stats_cout_jour = dept_stats.sort_values('CoutParJour', ascending=False)
        
        # Graphique 1: Coûts Totaux par Département - VERT (#10b981)
        fig_cout_dept = go.Figure(data=[
            go.Bar(
                y=dept_stats_cout['Departement'],
                x=dept_stats_cout['CoutTotal'],
                orientation='h',
                marker_color='#10b981',
                marker_line_color='#059669',
                marker_line_width=1.5,
                text=dept_stats_cout['CoutTotal'].apply(lambda x: format_number(x, currency=True, decimals=1)),
                textposition='outside',
                textfont=dict(color='#10b981', size=11),
                name='Coût Total'
            )
        ])
        fig_cout_dept.update_layout(
            title='',
        ...
        # Graphique 2: Durée Moyenne par Département - BLEU (#3b82f6)
        fig_duree_dept = go.Figure(data=[
            go.Bar(
                x=dept_stats_duree['Departement'],
                y=dept_stats_duree['DureeMoyenne'],
                marker_color='#3b82f6',
                marker_line_color='#2563eb',
                marker_line_width=1.5,
                text=dept_stats_duree['DureeMoyenne'].apply(lambda x: f'{x:.1f} j'),
                textposition='outside',
                textfont=dict(color='#3b82f6', size=11),
                name='Durée Moyenne'
            )
        ])
        fig_duree_dept.update_layout(
            title='',
        ...
        # Graphique 3: Coût par Jour par Département - VIOLET (#8b5cf6)
        fig_cout_jour = go.Figure(data=[
            go.Bar(
                x=dept_stats_cout_jour['Departement'],
                y=dept_stats_cout_jour['CoutParJour'],
                marker_color='#8b5cf6',
                marker_line_color='#7c3aed',
                marker_line_width=1.5,
                text=dept_stats_cout_jour['CoutParJour'].apply(lambda x: format_number(x, currency=True, decimals=1)),
                textposition='outside',
                textfont=dict(color='#8b5cf6', size=11),
                name='Coût par Jour'
            )
        ])
        fig_cout_jour.update_layout(
            title='',
            xaxis_title='Département',
            yaxis_title='Coût par Jour (€)',
            template='plotly_white',
            plot_bgcolor='white',
            paper_bgcolor='white',
            font=dict(color='#1a1f3a'),
        ...
        return fig_cout_dept, fig_duree_dept, fig_cout_jour
    
    # Callback pour le modal - détecte les clics sur les graphiques
    @app.callback(
        [Output('modal-dept', 'is_open'),
         Output('modal-dept-title', 'children'),
         Output('modal-dept-content', 'children')],
        [Input('graph-cout-departement', 'clickData'),
         Input('graph-duree-departement', 'clickData'),
         Input('graph-cout-jour-departement', 'clickData'),
         Input('close-modal-dept', 'n_clicks')],
    ...
    def toggle_modal(click_cout, click_duree, click_cout_jour, n_close, is_open, 
                     departement, maladie, traitement, sexe, age_min, age_max):
        """Gère l'ouverture/fermeture du modal avec les détails du département"""
        ...
        if trigger_id == 'close-modal-dept':
            return False, "", ""
        
        # Détecter quel graphique a été cliqué
        click_data = None
        if trigger_id == 'graph-cout-departement' and click_cout:
            click_data = click_cout
        elif trigger_id == 'graph-duree-departement' and click_duree:
            click_data = click_duree
        elif trigger_id == 'graph-cout-jour-departement' and click_cout_jour:
            click_data = click_cout_jour
        ...
        filtered_df = filter_data(df, departement, maladie, traitement, sexe, age_min, age_max)
        dept_df = filtered_df[filtered_df['Departement'] == dept_name]
        ...
        cout_total = dept_df['Cout'].sum()
        cout_moyen = dept_df['Cout'].mean()
        duree_moyenne = dept_df['DureeSejour'].mean()
        cout_par_jour = dept_df['CoutParJour'].mean()
        patients_risque = dept_df['PatientRisque'].sum()
        
        # Créer le contenu du modal
        modal_title = f"📊 Détails - {dept_name}"
